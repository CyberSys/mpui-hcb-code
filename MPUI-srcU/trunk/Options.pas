...
uses
  ...
  TntExtCtrls, TntComCtrls, TntStdCtrls, TntFileCtrl, ImgList;
...
begin
  ...
  if WideDirectoryExists(ELyric.Text) then Core.LyricDir:=ELyric.Text;
  if WideDirectoryExists(ESsf.Text) then Core.ShotDir:=ESsf.Text
  else if WideCreateDir(ESsf.Text) then Core.ShotDir:=ESsf.Text;
...
begin
  ...
  Config.Save(0);//(SystemDir+Config.DefaultFileName,0);
end;
