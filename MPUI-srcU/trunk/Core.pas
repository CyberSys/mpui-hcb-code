...
const MediaType:array[0..185]of WideString=(
...
begin
 ...
 // WorkareaH:=Workarea.Bottom-Workarea.Top;
...
begin
  ...
  sf:=CheckSubfont(subfont);
  if uof then begin
    if FileExists(sf) then CmdLine:=CmdLine+' -subfont '+EscapeParam(sf);
    sf:=CheckSubfont(osdfont);
  end;
  ...
  if Firstrun then begin
    ...
    for t:=Low(ZipType) to High(ZipType) do begin
      if WideFileExists(Vobfile+ZipType[t]) then begin
...
'=========================================================================='^M^J)
 ...
 // success:=createprocess(nil,pchar(utf8encode(cmdline)),nil,nil,true,detached_process,nil,pchar(utf8encode(shotdir)),si,pi);
